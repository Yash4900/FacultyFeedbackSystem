        ...
        while($row2=$res2->fetch_assoc()){
          ...
          if($q_id==$noOfQues){
            ...
            if($noOfStudents > 0)
              ...
              $avg_mid = number_format((float)($avg_mid), $pre,'.','');
...
<tr id="hdsj">
    ...
    <td id="avgmid"><?= $avg_mid ?></td>
    <td id="avgend"><?= $avg_end ?></td>
    <td id="avg"><?= $avg ?></td>
</tr>
<?php 
...
<script>
  function printDataSummary()
  {
   ...
   //newWin.document.write("<head><style>@media print{ table{border-collapse: collapse;} table, td, th{ border:1px solid #000; } </style></head>")
   newWin.print();
   newWin.close();
 }
